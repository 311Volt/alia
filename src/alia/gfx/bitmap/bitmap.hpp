#ifndef BITMAP_D13C36F5_B90B_4972_BABA_70EDC4AEE8DB
#define BITMAP_D13C36F5_B90B_4972_BABA_70EDC4AEE8DB
...
namespace alia {
    ...
    public:
        ...
        any_bitmap_view subview(this auto &&self, rect_i region) noexcept {
            ...
            std::byte *ptr = self.data_ + clamped.top() * self.stride_bytes_ + clamped.left() * bpp;
            return any_bitmap_view(self.pixel_format_, clamped.width(), clamped.height(), self.stride_bytes_, ptr);
        }
    ...
    public:
        ...
        auto line(this auto &&self, int y) noexcept {
            using SelfT = std::remove_reference_t<decltype(self)>;
            if constexpr (std::is_const_v<SelfT>)
                return std::span<const TPixel>(reinterpret_cast<const TPixel *>(self.data_ + y * self.stride_bytes_), self.width_);
            else
                return std::span<TPixel>(reinterpret_cast<TPixel *>(self.data_ + y * self.stride_bytes_), self.width_);
        }
        ...
        bitmap_view subview(this auto &&self, rect_i region) noexcept {
            const rect_i bounds{{0, 0}, {self.width_, self.height_}};
            const rect_i clamped = bounds.intersection_with(region);
            std::byte *ptr = self.data_ + clamped.top() * self.stride_bytes_ + clamped.left() * static_cast<int>(sizeof(TPixel));
    ...
    public:
        ...
        bitmap(pixel_format fmt, vec2i size, std::span<const std::byte> data, int stride_bytes = tight_pixel_packing)
            : pixel_format_(fmt), width_(size.x), height_(size.y),
        ...
        bitmap(vec2i size, std::span<const TPixel> pixels)
            : pixel_format_(TPixel::format_id), width_(size.x), height_(size.y), stride_bytes_(size.x * static_cast<int>(sizeof(TPixel))),
              data_(std::make_unique<std::byte[]>(static_cast<std::size_t>(stride_bytes_) * size.y)) {
        ...
        explicit bitmap(const any_bitmap_view &view)
            : pixel_format_(view.format()), width_(view.width()), height_(view.height()),
        ...
        explicit bitmap(const bitmap_view<TPixel> &view)
            : pixel_format_(TPixel::format_id), width_(view.width()), height_(view.height()),
        ...
        any_bitmap_view view(this auto &&self) noexcept {
            return any_bitmap_view(self.pixel_format_, self.width_, self.height_, self.stride_bytes_, self.data_.get());
        }
