...
namespace alia {
    ...
    void ogl_texture_impl::apply_sampler() noexcept {
        ...
        auto mag_filt = [&]() -> GLint { return (sampler_.mag_filter == texture_filter::nearest) ? GL_NEAREST : GL_LINEAR; };
    ...
    void ogl_texture_impl::unlock(const texture_lock_info &info, bool wrote) {
        ...
        glTexSubImage2D(GL_TEXTURE_2D, info.level, info.origin.x, info.origin.y, info.extent.x, info.extent.y, external, type, info.data);
    ...
    void ogl_texture_impl::generate_mipmaps() {
        ...
        glBindTexture(GL_TEXTURE_2D, tex_id);
#ifdef _WIN32
        static auto fn = reinterpret_cast<PFNGLGENERATEMIPMAPPROC>(wglGetProcAddress("glGenerateMipmap"));
#else
        static auto fn =
            reinterpret_cast<PFNGLGENERATEMIPMAPPROC>(glXGetProcAddressARB(reinterpret_cast<const GLubyte *>("glGenerateMipmap")));
#endif
        fn(GL_TEXTURE_2D);
    }
    ...
    std::unique_ptr<texture_impl> ogl_device_impl::create_texture(pixel_format fmt, vec2i size, int mip_levels) {
