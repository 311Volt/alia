...
namespace alia {
    ...
    static HWND create_dummy_hwnd() {
        ...
        return CreateWindowExW(0, cls, L"", WS_OVERLAPPEDWINDOW, 0, 0, 1, 1, nullptr, nullptr, GetModuleHandleW(nullptr), nullptr);
    }
    ...
    static std::unique_ptr<gfx_device_impl> create_d3d9_device() {
        ...
        HRESULT hr = d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, dummy, D3DCREATE_HARDWARE_VERTEXPROCESSING, &pp, &device);
        if (FAILED(hr)) {
            hr = d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, dummy, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &pp, &device);
        }
        if (FAILED(hr)) {
            DestroyWindow(dummy);
            d3d->Release();
    ...
    std::unique_ptr<swapchain_impl> d3d9_device_impl::create_swapchain(void *native_handle, vec2i initial_size) {
