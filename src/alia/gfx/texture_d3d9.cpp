...
namespace alia {
    ...
    static bool d3d9_supports_texture_format(IDirect3DDevice9 *device, D3DFORMAT fmt, DWORD usage) {
        ...
        const bool supported =
            ...
            SUCCEEDED(d3d->CheckDeviceFormat(params.AdapterOrdinal, params.DeviceType, display_mode.Format, usage, D3DRTYPE_TEXTURE, fmt));
    ...
    std::unique_ptr<texture_impl> d3d9_texture_impl::clone() const {
        ...
        for (int lv = 0; lv < mips_to_copy; ++lv) {
            ...
            if (FAILED(texture_->LockRect(static_cast<UINT>(lv), &src_lr, nullptr, D3DLOCK_READONLY))) {
                dst->Release();
                return nullptr;
            }
    ...
    std::unique_ptr<texture_impl> d3d9_device_impl::create_texture(pixel_format fmt, vec2i size, int mip_levels) {
        ...
        if (FAILED(device->CreateTexture(
                static_cast<UINT>(size.x), static_cast<UINT>(size.y), mips, usage, d3dfmt, D3DPOOL_MANAGED, &tex, nullptr
            )) ||
            !tex)
            return nullptr;
