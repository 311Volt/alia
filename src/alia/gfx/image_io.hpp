...
namespace alia {
	...
	struct image_loader_registrar {
		image_loader_registrar(std::string_view ext, image_loader_fn fn) {
			register_image_loader(ext, std::move(fn));
		}
	};

	/// RAII helper for registering an image writer during static initialization.
	struct image_writer_registrar {
		image_writer_registrar(std::string_view ext, image_writer_fn fn) {
			register_image_writer(ext, std::move(fn));
		}
	};
